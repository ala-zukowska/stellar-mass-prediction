...
def main():
    ...
    joined_X = joined_df[["L", "met"]]
    joined_M = joined_df[["M"]]
    ...
    model.fit(joined_X, joined_M)
    # M_pred = model.predict(X_test)

    cv = KFold(n_splits=5, shuffle=True, random_state=1)

    r2 = np.mean(cross_val_score(model, joined_X, joined_M, scoring='r2', cv=cv))
    mse = -np.mean(cross_val_score(model, joined_X, joined_M, scoring='neg_mean_squared_error', cv=cv))
    ...
    print(f"Cross using joined table validation \n MSE={mse:.3f}, r2={r2:.3f}")
